...
To uncover the latent skills underlying the eight-item test dataset, I employed a multi-method approach that combines Factor Analysis, K-Means Clustering, and Principal Component Analysis (PCA). Each method offers unique strengths and limitations, and by leveraging their complementary perspectives, I aimed to develop a more robust and comprehensive understanding of the knowledge structure underlying the data.
...
Factor Analysis was selected as the primary method for identifying latent skills due to its ability to uncover hidden constructs that explain the patterns of correlations among test items [@beavers2019practical]. I used exploratory factor analysis (EFA) with the principal axis factoring extraction method and an oblique rotation to allow for correlations among the latent skills.
...
To apply PCA, I first standardized the data to ensure that all items were on the same scale. I then conducted PCA using the correlation matrix and retained components with eigenvalues greater than one (Kaiser's criterion) [@chen2018knowedu]. I also examined the scree plot of eigenvalues to assess the relative importance of each component.
...
I extracted the item response data, excluding any non-item columns such as student identifiers. This step ensured that our analyses focused solely on the patterns of student responses across the eight test items.
...
To understand the item-skill relationships further, I created a Q-matrix based on the **Principal Component Analysis (PCA)** loadings. Each item is assigned to the skill (principal component) with which it has the highest loading.
...
I created a couple [Mermaid](https://mermaid.js.org/) diagrams to gain further insight.[^1]
...
Based on the consistency of results across methods, I developed a final Q-matrix that maps each item to its primary associated skill based on the three-factor EFA model. Table 8 presents the final Q-matrix, which shows a clear and interpretable mapping of items to skills.
