...
5. **Model Training & Evaluation**: Train the model on each training set, validate on a held-out portion, and evaluate on the corresponding test set using the Area Under the Receiver Operating Characteristic curve (AUC ROC) as the primary performance metric.
...
In the code above, `df` is the complete data frame containing both the annotation text and CP labels. The variable `y` is a Pandas Series containing my target variable (CP presence).
