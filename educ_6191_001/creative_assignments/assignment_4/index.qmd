...
To further assess the model’s generalizability, we conducted an additional evaluation on a subset of students who were entirely excluded from model training. By holding out a distinct group of previously unseen learners, we tested whether the model’s performance extended beyond the initially sampled population. This step is critical to assessing whether improvements in accuracy and AUC ROC metrics stem from a more robust embedding representation, rather than from overfitting to the training set or failing to capture generalizable aspects of process-oriented commentary.
